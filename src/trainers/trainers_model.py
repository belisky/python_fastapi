...
async def get_trainer_by_email(email: str) -> Optional[Trainer]:
    query = "SELECT first_name, email FROM trainers WHERE email = $1"
    async with database.pool.acquire() as connection:
        row = await connection.fetchrow(query, email)
        if row is not None:
            
            trainer = trainer(first_name=row["first_name"], email=row["email"]) 
            return trainer
        return None

async def get_all_trainers(limit: int, offset: int) -> List[Trainer]:
    query = "SELECT first_name, email FROM trainers LIMIT $1 OFFSET $2"
    async with database.pool.acquire() as connection:
        rows = await connection.fetch(query, limit, offset)
        print(rows)
        trainers = [Trainer(first_name=record["first_name"], email=record["email"]) for record in rows]
        return trainers
